...
func (h *TaskHandler) GetTasks(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.Query(
		`SELECT assignment_id, title, description, due_date, priority, status, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&task.AssignmentID, &task.Title, &task.Description, &task.DueDate, &task.Priority, &task.Status, &task.CreatedAt); err != nil {
			http.Error(w, "Failed to fetch tasks", http.StatusInternalServerError)
			return
		}
